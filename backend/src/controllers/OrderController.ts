...
const STRIPE_ENDPOINT = process.env.STRIPE_WEBHOOK_SECRET as string
...
type CheckoutSessionRequest = {
    ...
    deliveryDetails: { // Corrected from `deilveryDetails`
        email: string;
        name: string;
        addressLine1: string;
        city: string;
...
const getMyOrders = async (req: Request, res: any) => {
    try {
...
const createCheckoutSession = async (req: Request, res: any) => {
    try {
        ...
        const newOrder = new Order({
            ...
            user: req.userId,
            status: "placed",
            deliveryDetails: checkoutSessionRequest.deliveryDetails, // Corrected here
